...
describe('UpdatePermissionController', (): void => {
  ...
  it('Should be able to update a permission', async (): Promise<void> => {
    const response = await request(app.server).put(`/permissions/${id}`).send({
      name: 'updatedPermission',
    });

    expect(response.status).toBe(200);
    expect(response.body.data.name).toEqual('updatedPermission');
  });
});
